...
try {
    ...
    if ($conn) {
        ...
        $query = "SELECT i.*, t.id_trajet , e.id_etudiant, e.prenom_etudiant, e.nom_etudiant, vi1.nom_ville AS depart from inscription i
        ...
        if ($stmt->rowCount() > 0) {
            echo "<table><tr>";
            foreach (["Identifiant du trajet", "Identifiant du passager", "Nom du passager", "Prénom du passager", "Point de rendez-vous", "Destination", "Acceptation"] as $t) {
                echo "<th>" . $t . "</th>";
            }
            echo "</tr>";
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                echo "<tr>";
                foreach (["id_trajet", "id_etudiant", "nom_etudiant", "prenom_etudiant", "depart"] as $e) {
                    echo "<td>" . $row[$e] . "</td>";
                }
